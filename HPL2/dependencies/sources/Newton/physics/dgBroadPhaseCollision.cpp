...
{
	dgListNode* const node = (dgListNode*) body->m_collisionCell.m_axisArrayNode[dgInt32(m_index)];
	_ASSERTE (node);
...
//		dgListNode* entry;
		...
		sum += key;
		sum2 += key * key;
		for (prev = entry->GetPrev(); prev && (key < prev->GetInfo().m_key) ; prev = prev->GetPrev()) {
		}
...
//		dgFloat32 minVal = line.m_boxL0[m_index];
		...
		for (dgListNode* node = GetFirst(); node && (node->GetInfo().m_key < maxVal); node = node->GetNext() ) {
			minT = node->GetInfo().m_body->RayCast (line, filter, prefilter, userData, minT);
		}
	} else {
...
//		dgListNode* entry;
		...
		for (prev = entry->GetPrev(); prev && (prev->GetInfo().m_body->m_uniqueID > key) ; prev = prev->GetPrev()) {
		}
...
void dgBroadPhaseCell::UpdateAutoPair(dgWorld * const world, dgInt32 threadIndex)
{
//	dgInt32 indexX;
//	dgFloat32 maxVal;
//	dgBody *body0;
...
//	dgSortArray::dgListNode* innerNode;
	...
	for (dgSortArray::dgListNode* outerNode = lastSort->GetFirst(); outerNode; outerNode = outerNode->GetNext() ) {
		...
		if (!body0->m_collision->IsType (dgCollision::dgCollisionNull_RTTI)) {
			...
			for (dgSortArray::dgListNode* innerNode = outerNode->GetNext(); innerNode && innerNode->GetInfo().m_key <= maxVal; innerNode = innerNode->GetNext() ) {
...
dgBroadPhaseLayer::dgBroadPhaseLayer()
	:dgTree<dgBroadPhaseCell, dgUnsigned32>(NULL)
{
	m_me = NULL;
...
void dgBroadPhaseLayer::Init (dgWorld* const world, dgFloat32 cellSize, dgInt32 layerIndex)
{
	_ASSERTE (GetCount() == 0);
	m_me = world;
...
dgBroadPhaseCollision::dgBroadPhaseCollision(dgMemoryAllocator* allocator)
	:m_min (-dgFloat32(1000.0f), -dgFloat32(1000.0f), -dgFloat32(1000.0f), dgFloat32(0.0f)),
	 m_max ( dgFloat32(1000.0f),  dgFloat32(1000.0f), dgFloat32(1000.0f), dgFloat32(0.0f)), 
...
void dgBroadPhaseCollision::SetWorldSize (const dgVector& min, const dgVector& max)
{
	dgFloat32 cellSize;
	...
	cellSize = dgPow (dgFloat32 (2.0f), dgCeil (dgLog (cellSize) / dgLog (dgFloat32 (2.0f))));
...
//	dgBroadPhaseLayer::dgTreeNode* node;
	...
	if (!obtreeCell->m_count) {
		if (obtreeCell != &m_inactiveList) {
			dgBroadPhaseLayer::dgTreeNode* const node = m_layerMap[dgInt32(obtreeCell->m_layerIndex)].GetNodeFromInfo(*obtreeCell);
			_ASSERTE (node);
...
void dgBroadPhaseCollision::UpdatePairs (dgBroadPhaseCell& cellA, dgBroadPhaseCell& cellB, dgInt32 threadIndex) const
{
...
void dgBroadPhaseCollision::ForEachBodyInAABB (const dgVector& p0, const dgVector& p1, OnBodiesInAABB callback, void* const userdata) const
{
			...
			if (layerMap.GetCount()) {
				...
				for (dgInt32 xIndex = ix0; xIndex <= ix1; xIndex ++) {
					...
					for (dgInt32 zIndex = iz0; zIndex <= iz1; zIndex ++) {
						...
						if (cell) {
							for (dgSortArray::dgListNode *node = cell->m_sort[0].GetFirst(); node; node = node->GetNext()) {
...
dgInt32 dgBroadPhaseCollision::ConvexCast (
	dgCollision* const shape, 
	const dgMatrix& matrixOrigin, 
	const dgVector& target, 
	dgFloat32& timeToImpact,
	OnRayPrecastAction prefilter, 
	void* const userData, 
	dgConvexCastReturnInfo* const info,
	dgInt32 maxContacts,
		...
		for (dgInt32 layer = 0; layer < DG_OCTREE_MAX_DEPTH; layer ++) {
			if (m_layerMap[layer].GetCount()) {
				...
				for (dgInt32 xIndex = ix0; xIndex <= ix1; xIndex ++) {
					...
					dgInt32 iz1 = dgFastInt (z1 * cellScale);
					for (dgInt32 zIndex = iz0; zIndex <= iz1; zIndex ++) {
						dgBroadPhaseCell *const cell = m_layerMap[layer].Find (xIndex, zIndex);
						if (cell) {
							for (dgSortArray::dgListNode *node = cell->m_sort[0].GetFirst(); node; node = node->GetNext()) {
...
//									if (body != sentinel) {
									if (!body->m_collision->IsType(dgCollision::dgCollisionNull_RTTI)) {
										if (!PREFILTER_RAYCAST (prefilter, body, collision, userData)) {
											...
											if (cpu == dgSimdPresent) {
												count = me->CollideContinueSimd (collision, alignedMatrix, velocA, velocB,
																				   body->m_collision, body->m_matrix, velocB, velocB,
																				   time, points, normals, penetration, CONVEX_CAST_POOLSIZE, threadIndex);


											} else {
												count = me->CollideContinue (collision, alignedMatrix, velocA, velocB,
																			   body->m_collision, body->m_matrix, velocB, velocB,
																			   time, points, normals, penetration, CONVEX_CAST_POOLSIZE, threadIndex);
		...
		if (totalCount) {
			...
			dgVector dir (velocA.Scale (DG_RAY_TEST_LENGTH * dgRsqrt (velocA % velocA)));
		
			for (dgInt32 i = 0; i < totalCount; i ++) {
//				dgFloat32 t;
				dgContactPoint contact;
				dgVector p0 (info[i].m_point[0], info[i].m_point[1], info[i].m_point[2], dgFloat32 (0.0f));
				dgVector p1 (p0 + dir);
				p0 -= dir;
...
void dgBroadPhaseCalculateContactsWorkerThread::Realloc (dgInt32 jointsCount, dgInt32 contactCount, dgInt32 threadIndex)
{
	m_world->dgGetUserLock();

	dgCollidingPairCollector::dgPair *const pairs = m_world->m_pairs;
	dgContactPoint* const contactBuffer = (dgContactPoint*) m_world->m_contactBuffers[threadIndex];
	
	dgInt32 size = m_world->m_contactBuffersSizeInBytes[threadIndex] * 2;
	dgContactPoint* const newBuffer = (dgContactPoint*) m_world->GetAllocator()->MallocLow (size);  
...
void dgBroadPhaseCalculateContactsWorkerThread::ThreadExecute()
{
	...
	dgInt32 contactSize = dgInt32 (m_world->m_contactBuffersSizeInBytes[m_threadIndex] / sizeof (dgContactPoint));
	dgContactPoint* contactBuffer = (dgContactPoint*) m_world->m_contactBuffers[m_threadIndex];
	...
	if (m_useSimd) {
		for (dgInt32 i = 0; i < count; i += step) {
			...
			if ((contactIndex + DG_MAX_CONTATCS) > contactSize) {
				...
				contactSize = dgInt32 (m_world->m_contactBuffersSizeInBytes[m_threadIndex] / sizeof (dgContactPoint));
				contactBuffer = (dgContactPoint*) m_world->m_contactBuffers[m_threadIndex];
			}
...
void dgBroadPhaseMaterialCallbackWorkerThread::ThreadExecute()
{
	...
	for (dgInt32 i = 0; i < count; i += step) {
		...
		if (pair.m_contactCount || pair.m_isTrigger) {
			...
			if (!pair.m_contactBuffer) {
				m_world->ProcessCachedContacts (pair.m_contact, pair.m_material, m_timestep, m_threadIndex);
			} else {
	...
	if (filter && (dist2 > dgFloat32 (1.0e-8f)) && dgRayBoxClip (ll0, ll1, m_appMinBox, m_appMaxBox)) {
		...
		for (dgInt32 i = 0; i < DG_OCTREE_MAX_DEPTH; i ++) {
			...
			if (layer.GetCount()) {
				...
				dgVector boxP0 (GetMax (x0, dgFloat32 (0.0f)), GetMax (y0, dgFloat32 (0.0f)), GetMax (z0, dgFloat32 (0.0f)), dgFloat32 (0.0f));
				dgVector boxP1 (GetMin (x1, m_boxSize.m_x),    GetMin (y1, m_boxSize.m_y),    GetMin (z1, m_boxSize.m_z),    dgFloat32 (0.0f));

				dgVector dq (rayP1.m_x - rayP0.m_x, rayP1.m_y - rayP0.m_y, rayP1.m_z - rayP0.m_z, dgFloat32 (0.0f));
				...
				if (dgRayBoxClip (p0, p1, boxP0, boxP1)) {
					...
					do {
						...
						if (cell) {
							...
							minT = cell->m_lastSortArray->RayCast (minT, line, filter, prefilter, userData);
						}
						if (txAcc < tzAcc) {
							xIndex0 += xInc;
							tx = txAcc;
...
//			_ASSERTE (cell);
	...
	if (body->m_isInWorld) {
		...
		dgWorld* const me = (dgWorld*) this;
		if ((body->m_minAABB.m_x > m_appMinBox.m_x) && (body->m_minAABB.m_y > m_appMinBox.m_y) && (body->m_minAABB.m_z > m_appMinBox.m_z) && 
			(body->m_maxAABB.m_x < m_appMaxBox.m_x) && (body->m_maxAABB.m_y < m_appMaxBox.m_y) && (body->m_maxAABB.m_z < m_appMaxBox.m_z)) {  
			...
			for (dgInt32 layer = DG_OCTREE_MAX_DEPTH - 1; layer >= 0; layer --) {
				...
				if (ix1 == ix0) {
					...
					if (iz1 == iz0) {
						...
						dgBroadPhaseCell* const newCell = m_layerMap[layer].FindCreate (ix0,  iz0);
						newCell->m_active = 1;
						dgBroadPhaseCell* const cell = body->m_collisionCell.m_cell;
						if (newCell != cell) {
							cell->Remove(body);
							if (!cell->m_count) {
								dgBroadPhaseLayer::dgTreeNode* const node = m_layerMap[dgInt32(cell->m_layerIndex)].GetNodeFromInfo(*cell);
								_ASSERTE (node);
...
void dgBroadPhaseCollision::UpdateContactsSimd (dgFloat32 timestep, bool collisioUpdate)
{
#ifdef DG_BUILD_SIMD_CODE
	dgWorld* const me = (dgWorld*) this;
	dgUnsigned32 ticks = me->m_getPerformanceCount();
	dgUnsigned32 narrowTicks = UpdateContactsBroadPhaseBegin(timestep, collisioUpdate, ticks);
	...
	if (threadCounts > 1) {
		...
		for (dgInt32 threadIndex = 0; threadIndex < threadCounts; threadIndex ++) {
			...
			m_calculateContactsWorkerThreads[threadIndex].m_count = chunkSizes[threadIndex] * threadCounts;
			...
			me->m_threadsManager.SubmitJob(&m_calculateContactsWorkerThreads[threadIndex]);
		}
		me->m_threadsManager.SynchronizationBarrier ();
		...
		for (dgInt32 threadIndex = 0; threadIndex < threadCounts; threadIndex ++) {
			...
			m_materialCallbackWorkerThreads[threadIndex].m_count = chunkSizes[threadIndex] * threadCounts;;
			m_materialCallbackWorkerThreads[threadIndex].m_pairs = &pairs[threadIndex];
			...
			me->m_threadsManager.SubmitJob(&m_materialCallbackWorkerThreads[threadIndex]);
		}
		me->m_threadsManager.SynchronizationBarrier ();
...
void dgBroadPhaseCollision::UpdateContacts (dgFloat32 timestep, bool collisioUpdate)
{
