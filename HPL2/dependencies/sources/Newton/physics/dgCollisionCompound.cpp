...
dgCollisionCompound::OOBBTestData::OOBBTestData (const dgMatrix& matrix, const dgVector& p0, const dgVector& p1)
...
bool dgCollisionCompound::dgNodeBase::BoxTest (const OOBBTestData& data, const dgNodeBase* const otherNode) const
{
...
dgFloat32 dgCollisionCompound::dgNodeBase::BoxClosestDistance (const dgVector* const points, dgInt32 count) const
{
	dgVector box[8];
...
dgCollisionCompound::dgCollisionCompound(dgWorld* world)
...
dgCollisionCompound::dgCollisionCompound (const dgCollisionCompound& source)
	:dgCollision (source.GetAllocator(), 0, dgGetIdentityMatrix(), m_compoundCollision) 
{
	int stack;
...
void dgCollisionCompound::Init (dgInt32 count, dgCollisionConvex* const shapeArray[])
{
	m_count = count;
	...
	m_boxMinRadius = GetMin(m_root->m_size.m_x, m_root->m_size.m_y, m_root->m_size.m_z);
...
dgFloat32 dgCollisionCompound::RayCast (const dgVector& localP0, const dgVector& localP1, dgContactPoint& contactOut, OnRayPrecastAction preFilter, const dgBody* const body, void* const userData) const
{
	...
	while (stack) {
		...
		if (me && ray.BoxTest (me->m_p0, me->m_p1)) {
			if (me->m_type == m_leaf) {
				...
				param = shape->RayCast (p0, p1, tmpContactOut, preFilter, body, userData);
				if (param < maxParam) {
					maxParam = param;
					contactOut.m_normal = shape->m_offset.RotateVector (tmpContactOut.m_normal);;
...
dgFloat32 dgCollisionCompound::RayCastSimd (const dgVector& localP0, const dgVector& localP1, dgContactPoint& contactOut, OnRayPrecastAction preFilter, const dgBody* const body, void* const userData) const
{
	const dgNodeBase* stackPool[DG_COMPOUND_STACK_DEPTH];
	...
	while (stack) {
		...
		if (me && ray.BoxTestSimd (me->m_p0, me->m_p1)) {
			if (me->m_type == m_leaf) {
				...
				dgFloat32 param = shape->RayCastSimd (p0, p1, tmpContactOut, preFilter, body, userData);
				if (param < maxParam) {
					maxParam = param;
					contactOut.m_normal = shape->m_offset.RotateVector (tmpContactOut.m_normal);
					contactOut.m_userId = tmpContactOut.m_userId;
...
//		dgVector vol (m_array[i]->CalculateVolumeIntegral (m_collisionMatrix[i], bouyancyPlane, context));
		dgVector vol (m_array[i]->CalculateVolumeIntegral (matrix, bouyancyPlane, context));
...
void dgCollisionCompound::CalculateInertia (dgVector& inertia, dgVector& origin) const
{
	dgInt32 i;
	...
	for (i = 0; i < m_count; i ++) {
		collision = m_array[i];
		totalVolume += collision->CalculateMassProperties (tmpInertia, tmpCrossInertia, tmpOrigin);
...
bool dgCollisionCompound::OOBBTest (const dgMatrix& matrix, const dgCollisionConvex* const shape, void* const cacheOrder) const
{
	_ASSERTE (0);
	return true;
}
...
dgCollisionCompound::dgNodeBase* dgCollisionCompound::BuildTopDownTree(dgInt32 count, dgNodeBase** const proxiArray, dgInt32& id)
{
...
void dgCollisionCompound::PushNodes (dgNodeBase* const root, dgNodeBase** const proxiArray, dgInt32& index) const
{
	if (root->m_left) {
...
{
	minBox = dgVector (GetMin (node0->m_p0.m_x, node1->m_p0.m_x), GetMin (node0->m_p0.m_y, node1->m_p0.m_y), GetMin (node0->m_p0.m_z, node1->m_p0.m_z), dgFloat32 (0.0f));
	maxBox = dgVector (GetMax (node0->m_p1.m_x, node1->m_p1.m_x), GetMax (node0->m_p1.m_y, node1->m_p1.m_y), GetMax (node0->m_p1.m_z, node1->m_p1.m_z), dgFloat32 (0.0f));		
...
dgCollisionCompound::dgNodeBase* dgCollisionCompound::BuildTree(dgInt32 count, dgCollisionConvex* const shapeArray[])
{
...
//					dgShapeCell* const otherCell = (dgShapeCell*)other;
...
//					if (otherCell->m_lru != lru) {
//						m_world->dgGetIndirectLock(&otherCell->m_criticalSection);
//						otherCell->m_lru = lru;
//						otherCell->m_collisionMatrix = otherCell->m_shape->m_offset * otherMatrix;
...
//				dgNode* const myNode = (dgNode*)me;
			...
			} else if (other->m_area > me->m_area) {
//				dgNode* const otherNode = (dgNode*)other;
				_ASSERTE (me->m_type == m_node);
				_ASSERTE (other->m_type == m_node);
...
//	dgInt32 lru = m_world->m_broadPhaseLru;
	...
	while (stack) {
		...
		dgVector size (nodeProxi.m_size.m_y, nodeProxi.m_size.m_z, nodeProxi.m_size.m_x, dgFloat32 (0.0f));
