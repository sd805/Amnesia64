	...
	m_boxOrigin (dgFloat32 (0.0f), dgFloat32 (0.0f), dgFloat32 (0.0f), dgFloat32 (1.0f))
{
	...
	for (dgUnsigned32 i = 0; i < sizeof (m_supportVertexStarCuadrant) / sizeof (m_supportVertexStarCuadrant[0]); i ++) {
...
void dgCollisionConvex::SetVolumeAndCG ()
{
	...
	for (i = 0; i < m_edgeCount; i ++) {
		...
		if (!edgeMarks[i]) {
			...
			do {
				...
				count ++;
				_ASSERTE (count < sizeof (faceVertex) / sizeof (faceVertex[0]));
				edge = edge->m_next;
			} while (edge != face);
	...
	_ASSERTE (m_supportVertexStarCuadrant[5] == GetSupportEdge (m_multiResDir[1].Scale (-1.0f)));
	_ASSERTE (m_supportVertexStarCuadrant[6] == GetSupportEdge (m_multiResDir[2].Scale (-1.0f)));
	_ASSERTE (m_supportVertexStarCuadrant[7] == GetSupportEdge (m_multiResDir[3].Scale (-1.0f)));
...
void dgCollisionConvex::CalculateInertia (void* userData, int indexCount, const dgFloat32* faceVertex, int faceId)
{
...
dgFloat32 dgCollisionConvex::CalculateMassProperties (dgVector& inertia, dgVector& crossInertia, dgVector& centerOfMass) const
{
	dgFloat32 volume;
...
{ 
	...
	if (inertiaOut.m_x < dgFloat32 (1.0e-3f)) inertiaOut.m_x = dgFloat32 (1.0e-3f);
	if (inertiaOut.m_y < dgFloat32 (1.0e-3f)) inertiaOut.m_y = dgFloat32 (1.0e-3f);
	if (inertiaOut.m_z < dgFloat32 (1.0e-3f)) inertiaOut.m_z = dgFloat32 (1.0e-3f);
...
bool dgCollisionConvex::RayHitBox (const dgVector& localP0, const dgVector& localP1) const
{
	dgFloat32 tMin;
	dgFloat32 tMax;
...
bool dgCollisionConvex::OOBBTest (const dgMatrix& matrix, const dgCollisionConvex* const shape, void* const cacheOrder) const
{
	for (dgInt32 i = 0; i < 3; i ++) {
		dgVector dir (matrix.m_front[i], matrix.m_up[i], matrix.m_right[i], dgFloat32 (0.0f));
